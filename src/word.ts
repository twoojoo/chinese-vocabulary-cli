...
word.command("test")
	...
	.action(async (options) => {
		try {
			...
			if (errors["chinese-pinyin"].length > 0) console.log(RED_CROSS, `Chinese → Pinyin errors: ${errors["chinese-pinyin"].length} words`)
			if (errors["chinese-english"].length > 0) console.log(RED_CROSS, `Chinese → English errors: ${errors["chinese-english"].length} words`)
			if (errors["english-chinese"].length > 0) console.log(RED_CROSS, `English → Chinese errors: ${errors["english-chinese"].length} words`)
			if (errors["english-pinyin"].length > 0) console.log(RED_CROSS, `English → Pinyin errors: ${errors["english-pinyin"].length} words`)
		} catch (err: any) {
			console.error(err)
			console.error(err.message)
		}
